...
const Navbar = () => {
  ...
  // Auth from AuthProvider (no AppContext here)
  const { user, logout } = useAuth(); // user: {_id, name, email, role, image?}

  const handleLogout = () => {
    logout();              // clears token/user in AuthProvider
    navigate("/login");    // send to login
  };

  // Small helper: avatar initial if no user.image
  const initials =
    user?.name?.trim()?.split(" ")?.map(w => w[0]?.toUpperCase())?.slice(0, 2).join("") ||
    "U";
      ...
      <div className="flex items-center gap-4">
        {user ? (
          ...
          <div className="flex items-center gap-2 cursor-pointer group relative">
            {/* Avatar: image if present, else initials bubble */}
            {user.image ? (
            ...
            <div className="absolute top-0 right-0 pt-14 text-base font-medium text-gray-600 z-20 hidden group-hover:block">
              <div className="min-w-48 bg-gray-50 rounded shadow-lg flex flex-col gap-4 p-4">
                ...
                {user.role === "admin" && (
                  <p onClick={() => navigate("/admin")} className="hover:text-black cursor-pointer text-red-600">
                    Admin Panel
                  </p>
                )}
